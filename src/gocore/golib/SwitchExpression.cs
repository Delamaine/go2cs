...
namespace go
{
    ...
    {
        public SwitchExpression(in T value) => Value = value;
    ...
    public static class SwitchExpressionExtensions
    {
        ...
        private static bool IsTypeMatch<T>(SwitchExpression<T> target, Type type)
        {
            ...
            if (typeAssert is null)
                return false;

            try
            {
                return (bool)(typeAssert.Invoke(null, new object?[] { target.Value, type }) ?? false);
            }
            catch
            {
                return false;
        ...
        {
            return target.Case(() => testValues.Any(testValue => testValue?.Equals(target.Value) ?? target.Value is null), action, fallThrough);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static SwitchExpression<T> Case<T>(this SwitchExpression<T> target, T testValue, Action action, bool fallThrough)
        {
            return target.Case(() => testValue?.Equals(target.Value) ?? target.Value is null, action, fallThrough);
        }
